...
describe("<LatestMovieReviewsContainer />", () => {
  ...
  beforeEach(() => {
    ...
    wrapper = !LatestMovieReviewsContainer.prototype
      ? shallow(<Noop />)
      : shallow(<LatestMovieReviewsContainer />);
  });
  ...
  it("should have state", () => {
    ...
    expect(
      LatestMovieReviewsContainer.prototype,
  ...
  it("should fetch data from the New York Times API", () => {
    ...
    expect(
      fetchSpy.firstCall.lastArg,
      "Fetch should have the base URL 'https://api.nytimes.com/svc/movies/v2/reviews/all.json?'"
    ).to.include("https://api.nytimes.com/svc/movies/v2/reviews/all.json?");
  });

  it("should render reviews after reviews state updated", () => {
    wrapper = !LatestMovieReviewsContainer.prototype
      ? mount(<Noop />)
      : mount(<LatestMovieReviewsContainer />);

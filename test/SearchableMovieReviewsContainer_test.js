...
describe("<SearchableMovieReviewsContainer />", () => {
  ...
  it("should have state", () => {
    ...
    expect(
      SearchableMovieReviewsContainer.prototype,
  ...
  it("should fetch data from the New York Times API on form submission", () => {
    ...
    expect(
      ...
      "Fetch should have the base URL 'https://api.nytimes.com/svc/movies/v2/reviews/search.json?'"
    ).to.include("https://api.nytimes.com/svc/movies/v2/reviews/search.json?");
  });

  it("should render reviews after reviews state updated", () => {
    wrapper = !SearchableMovieReviewsContainer.prototype
      ? mount(<Noop />)
      : mount(<SearchableMovieReviewsContainer />);
